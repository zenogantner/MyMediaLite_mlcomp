...
if [ "$1" == "construct" ]; then
    ...
    xargs --arg-file=hp.txt mono --debug RatingPrediction.exe --training-file=$2 --recommender=$recommender --recommender-options="num_factors=$k learn_rate=0.005 num_iter=125" --save-model=model.txt
    cp $2 training_file
elif [ "$1" == "predict" ]; then
