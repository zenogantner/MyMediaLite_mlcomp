...
if [ "$1" == "construct" ]; then
    ...
    xargs --arg-file=hp.txt mono --debug RatingPrediction.exe --training-file=$2 --recommender=$recommender --recommender-options="reg_u=12 reg_i=2 shrinkage=50" --save-model=model.txt
    cp $2 training_file
elif [ "$1" == "predict" ]; then
